...
- [x] 建立统一错误响应契约：`errorCode`、`traceId`、`params`、`fieldErrors`。
- [x] 前端 API 错误统一转为 i18n-aware `AppError`。

## 待完成 Todo

### 后端基础

- [x] 完善统一异常处理第一期：业务异常、参数异常、未登录、无权限、未知异常。
- [x] 为错误响应补充稳定错误码。
- [x] 增加 `traceId`，用于排查服务端异常。
...
- [x] 建立全局视觉变量和通用工作台样式。
- [x] 为 API 客户端补充统一登录失效错误码处理。
- [x] 为 API 客户端补充统一错误消息策略。
- [x] 增加共享 API 响应和错误类型。
...
- 异常提示第一期完成：后端稳定错误码和 traceId，前端按当前语言翻译 API 错误。
```
