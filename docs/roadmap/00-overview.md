...
| 07 | Storage and Media | 多存储第一期已完成，博客图片接入和孤儿清理任务未完成 |
...
- 多存储第一期：LOCAL、腾讯 COS、阿里 OSS、Cloudflare R2 配置，工作台测试/切换，文件引用和孤儿标记底座。

## 当前最重要的缺口

- 公开博客和项目展示还没有独立页面与数据模型。
- 博客图片还没有接入文件引用同步，孤儿文件清理任务未完成。
...
2. 补齐 02 阶段：博客列表、博客详情、项目展示，并接入 07 阶段的图片引用同步。
