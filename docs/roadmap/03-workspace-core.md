...
当前状态：公开个人主页、站点配置、认证链路、RBAC 权限基础、权限管理、课程管理、班级管理、学期管理、模板管理、文件中心、生成记录查询和工作台主布局进行中，已有 Token 登录接口、角色/权限返回、用户角色分配、站点公开配置、课程/班级/学期/模板/文件中心/生成记录前后端和响应式工作台壳层，学期周历支持生成后人工微调。最后核查时间：2026-07-02。
...
- [x] 前端实现退出登录
- [x] 登录用户返回角色和权限
- [x] 初始化 OWNER / ADMIN / EDITOR / VIEWER 角色和常用权限
- [x] 工作台 API 支持 `@RequirePermission` 权限拦截
- [x] 实现用户列表和角色列表管理 API
- [x] 实现用户角色分配 API
- [x] 防止移除系统最后一个 OWNER 账号

### 公开站点与访客首页

- [x] 创建 `site_config` 数据表迁移
- [x] 初始化站点默认配置
- [x] 实现 `GET /api/public/site`
- [x] 实现 `GET /api/site/config`
- [x] 实现 `PUT /api/site/config`
- [x] 后台站点配置 API 使用 `site:config` 权限保护
- [x] 将 `/` 改造为公开个人主页骨架
- [x] 将首页简化为个人主页风格首屏
- [x] 首页按配置开关显示个人简介、博客入口、项目展示和工作台入口
- [x] 首页保留控制台，并按权限控制命令是否可执行
- [x] 创建 `/workspace/site` 站点配置页面
- [ ] 实现博客文章表和公开博客列表
- [ ] 实现项目展示表和公开项目列表
