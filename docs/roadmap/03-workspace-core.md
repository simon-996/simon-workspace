...
当前状态：认证链路、课程管理后端、班级管理后端、学期管理后端、模板管理后端、文件中心后端、生成记录查询后端和工作台主布局进行中，已有简单 Token 登录接口、课程/班级 CRUD 接口、学期与周历接口、模板与字段接口、文件中心接口、生成记录查询接口和响应式工作台壳层。最后核查时间：2026-06-21。
